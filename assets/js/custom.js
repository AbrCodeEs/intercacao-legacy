...
if(fitToSection_flv == true || fitToSection_flv == "true"){
	...
	owlTeam.owlCarousel({
		dots: false, nav: true, navText: Array('&lsaquo;','&rsaquo;'),
		responsiveClass: true,
		responsive:{
			0:{
				items:1
...
var mylist = jQuery('div.megafolio-container');
		var divItems = mylist.children('div.mega-entry');
		var listitems = divItems.get();
		
				
		var category = jQuery(this).data('category');
		var orderby = 'md-order-'+category;
		
		
		if(category=='cat-6'||category=='cat-8'||category=='cat-33'){ //colombia o venezuela o peru
				//no ordenar
		}else{

		divItems.removeClass('tp-layout-last-item very-last-item tp-layout-first-item');

		
			listitems.sort(function(a,b){
				var compA = Number(jQuery('div.'+orderby,a).text().trim());
				var compB = Number(jQuery('div.'+orderby,b).text().trim());
				//console.log('***** orden por '+orderby+'=='+ jQuery('div.mega-hovertitle',a).text().substring(0,10)+'=vs='+ jQuery('div.mega-hovertitle',b).text().substring(0,10)+'=== '+compA+'  vs '+compB);
				jQuery('div.'+orderby,a).data('ordenadopor',compA);
				jQuery('div.'+orderby,b).data('ordenadopor',compB);
				return (compA > compB) ? -1 : (compA < compB) ? 1 : 0;
			});
			jQuery('div.mega-metainf').remove();
		
		
			jQuery.each(listitems, function(idx, itm) {  
				if(idx%3==0){jQuery(itm).addClass('tp-layout-first-item');}
				if(idx%3==2){jQuery(itm).addClass('tp-layout-last-item');} 
				if(idx==listitems.length-1){jQuery(itm).addClass('very-last-item');}
				
				if(category=='cat-7'){
				valor = jQuery('div.md-order-rank',itm).text().trim();
					jQuery('div.mega-entry-innerwrap',itm).prepend("<div class='mega-metainf'>"+valor+" <span class='star'>&#9734;</span></div>")	
				}else
					if(category=='cat-23'||category=='cat-24'){ //criollo o fermentado
					valor = jQuery('div.'+orderby,itm).text().trim();
						jQuery('div.mega-entry-innerwrap',itm).prepend("<div class='mega-metainf'>"+valor+"%</div>")	
					}else
					if(category=='cat-6'||category=='cat-8'||category=='cat-33'){ //colombia o venezuela o peru
						...
						if(category=='cat-26'){
							positiveClass="<span class='positive-icon mega-sweet_fruit'>&nbsp;</span>";
						}
						if(category=='cat-27'){
							positiveClass="<span class='positive-icon mega-soar_fruit'>&nbsp;</span>";
						}
						if(category=='cat-28'){
							positiveClass="<span class='positive-icon mega-floral'>&nbsp;</span>";
						}
						if(category=='cat-29'){
							positiveClass="<span class='positive-icon mega-criollo'>&nbsp;</span>";
						}
						if(category=='cat-30'){
							positiveClass="<span class='positive-icon mega-cocoa'>&nbsp;</span>";
						}
						if(category=='cat-31'){
							positiveClass="<span class='positive-icon mega-forest'>&nbsp;</span>";
						}
						if(category=='cat-32'){
							positiveClass="<span class='positive-icon mega-herbal'>&nbsp;</span>";
						}
						if(category=='cat-5'){
							positiveClass="<span class='positive-icon mega-certified'>&nbsp;</span>";
							if(valor=="")
								valor="No";
						}
						if(category=='cat-34'){
							positiveClass="<span class='positive-icon mega-awarded'>&nbsp;</span>";
							if(valor=="")
								valor="0";
						}
						
						jQuery('div.mega-entry-innerwrap',itm).prepend("<div class='mega-metainf'>"+positiveClass+" "+valor+"</div>");
					}			
				
				mylist.append(itm);
				
				//console.log(jQuery('div.mega-hovertitle',itm).text().substring(0,10)+" = "+jQuery('div.'+orderby,itm).text());
